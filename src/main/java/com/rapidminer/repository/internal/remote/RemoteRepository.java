...
public interface RemoteRepository extends Repository, RemoteFolder {
    ...
    /**
     * The constant TAG_REMOTE_REPOSITORY.
     */
    ...
    public static enum EntryStreamType {
        /**
         * Metadata entry stream type.
         */
        METADATA, /**
         * Ioobject entry stream type.
         */
        IOOBJECT, /**
         * Process entry stream type.
         */
        PROCESS, /**
         * Blob entry stream type.
         */
        BLOB
	}
    ...
    /**
     * Sets passwort input canceled.
     *
     * @param pwCanceled sets the state of password input canceled
     */
    void setPasswortInputCanceled(boolean pwCanceled);

    /**
     * Is password input canceled boolean.
     *
     ...
     *
     * @return {@code true} if the server is reachable at the moment with the current password and a         sufficient server version was found, {@code false} if the login dialog was canceled
     * @throws RepositoryException if the connection failed
     */
     ...
     *
     * @return the {@link RemoteInfoService} if it can be accessed. If the queried Server cannot be         reached or has no {@link RemoteInfoService} or the login dialog was canceled         <code>null</code> is returned.
     */
     ...
     * @throws RepositoryException            if the connection failed
     * @throws PasswordInputCanceledException if the login dialog was canceled
     */
     ...
     *
     * @throws RepositoryException            in case the content manager could not be created
     ...
     * @param preAuthHeader if {@code true} the username and password is added to the connection if present,            if {@code false} it is checked if the RM server is reachable
     ...
     * @return the connection
     * @throws IOException         the io exception
     * @throws RepositoryException if preAuthHeader is {@code false} and checking if the server is reachable failed             or if the login dialog was canceled during this check
     */
     ...
     *
     * @param listener the {@link ConnectionListener} to register
     */
