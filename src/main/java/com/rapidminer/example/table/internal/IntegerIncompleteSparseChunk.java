...
final class IntegerIncompleteSparseChunk extends IntegerIncompleteChunk {
    ...
    /**
     * Instantiates a new Integer incomplete sparse chunk.
     *
     * @param id           the id
     ...
     */
    IntegerIncompleteSparseChunk(int id, IntegerIncompleteChunk[] chunks, double defaultValue, DataManagement management) {
	...
	private void changeToDense(int max) {
		...
		IntegerIncompleteChunk dense = new IntegerIncompleteDenseChunk(id, chunks, ensuredSize, testingDefaultValue,
				management);
		for (int i = 0; i <= max; i++) {
			dense.set(i, get(i));
		}
